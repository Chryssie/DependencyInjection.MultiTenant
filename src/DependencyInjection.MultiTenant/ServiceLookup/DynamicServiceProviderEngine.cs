...
namespace Microsoft.Extensions.DependencyInjection.ServiceLookup
{
    internal sealed class DynamicServiceProviderEngine : CompiledServiceProviderEngine {
        private readonly MultiTenantServiceProvider _serviceProvider;

        public DynamicServiceProviderEngine(MultiTenantServiceProvider serviceProvider): base(serviceProvider)
        {
			this._serviceProvider = serviceProvider;
        }

        public override Func<MultiTenantProviderEngineScope, object> RealizeService(ServiceCallSite callSite)
        {
            var callCount = 0;
            return scope =>
            {
                ...
                if (Interlocked.Increment(ref callCount) == 2)
                {
                    ...
                    _ = ThreadPool.UnsafeQueueUserWorkItem(_ =>
                    {
                        try
                        {
							this._serviceProvider.ReplaceServiceAccessor(callSite, base.RealizeService(callSite));
                        }
                        catch (Exception ex)
                        {
